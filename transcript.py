...
- Allows to remove repetitive headers (and footers) from each page based on common pattern repeated accross topmost elements producing a continuous as opposed to a paged document.
- Allows to remove repetitive passages
- Reduces code
- Batch processing (pdf2html.py is the 1st step of the batch process)
- Configurable (See config.py for options)
...
DEBUG = 0
# remove spans less then this width (in px)
MIN_SPAN_SIZE = 8
# lines over this height indicate new paragraphs
MAX_LINE_HEIGHT = 18
# reconstruct tables
TABLES = 1
# remove styles when done to show off the naked semantic HTML and get ready for custom CSS, also removes paging data attributes
STRIP_CSS = 1
# place break rules at original line endings
BR = 1
REMOVE_HEADERS = 1
# list item bullets
BULLETS = ('•', '○', '■')
...
if __name__ == '__main__':
    ...
    for i, path in enumerate(glob.glob(config.DATA_DIR + '/*.html')):
        batch_process(path)
